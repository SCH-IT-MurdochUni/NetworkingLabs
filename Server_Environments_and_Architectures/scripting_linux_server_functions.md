...
== Challenge: Making a Server Tweet ==

Start by creating a twitter account. You can generate a very anonymous twitter account if required. 

After you have created an account, you will generate a key that you can use to post to twitter here: https://developer.twitter.com/apps

Browse to https://developer.twitter.com/apps Add a name for the application and a description. Where it says website, enter:

 http://127.0.0.1

Then click create your Twitter application. On the next screen find "Keys and Access Tokens". Click Create my access token at the bottom of the page.

Now save all the details as we will need them shortly (file-save-as or keep the browser window open and complete the next steps in a new tab).

Next, we will install the necessary software for Linux to communicate with the Twitter server.
...
Then edit the key file
 ...
 nano tweet.client.key

Enter the details that you copied from the access token into the appropriate fields of this file. 
*The My Language field should just be entered as en. 
*The screen name is your app name. 

Then, run the program:

 ./tweet.sh post hello world

If you get an error resembling FATAL ERROR: A required command then make sure that you install the required packages.

 sudo apt install 'missing_package_names'

Then try again.

 ./tweet.sh post hello world 

Check your twitter page to find your post. Think carefully about how the text that you posted, could be anything. This is just bash scripting so it could post a login attempt, or the cpuload or the temperature of the computer. You would only be limited by what you can do with bash.

As an example try putting the following in a script and executing it:

 now=$(date)
 echo $now
 ./tweet.sh post $now

Execute this code and then check your twitter feed.
